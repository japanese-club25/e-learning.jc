      ...
      {scanning && (
        <div className="fixed inset-0 z-[100] bg-black">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
            <div className="relative h-64 w-64 max-w-[75vw] max-h-[75vw]">
            ...
            <p className="mt-8 text-lg font-semibold text-white">Place the QR code inside the frame</p>
            <p className="mt-2 text-sm text-white/70">Keep the code steady until it is detected</p>
          </div>
          <button type="button" onClick={stop} aria-label="Close scanner" className="absolute right-5 top-5 rounded-full bg-black/50 p-3 text-white backdrop-blur hover:bg-orange-500">
            <X className="h-6 w-6" />
          </button>
          {busy && <div className="absolute bottom-8 left-1/2 -translate-x-1/2 rounded-full bg-orange-500 px-5 py-2 text-sm font-semibold text-white">Checking...</div>}
        </div>
      )}
    </div>
  );
